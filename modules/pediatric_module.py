"""
Pediatric Pneumonia Modality Module — Hirva (Client 4)
Dataset: Labeled Chest X-Ray Images (Guangzhou Women and Children's Medical Center)
(Kaggle: tolgadincer/labeled-chest-xray-images)
...
def get_model() -> torch.nn.Module:
    """Returns ResNet18 model configured with 2-class binary classification head for Pediatric Pneumonia."""
    try:
...
def get_hospital_partitions(num_hospitals: int = 3, alpha: float = 0.5) -> List[Tuple[DataLoader, DataLoader]]:
    """Partition Pediatric Pneumonia dataset across hospital clients."""
    image_paths = []
    labels = []
...
if __name__ == "__main__":
    ...
    print("Pediatric Pneumonia Module Verification complete! Metrics:", metrics)

    os.makedirs("checkpoints", exist_ok=True)
    torch.save(m.state_dict(), "checkpoints/pediatric_best_model.pth")
    print("Saved checkpoint to checkpoints/pediatric_best_model.pth")
